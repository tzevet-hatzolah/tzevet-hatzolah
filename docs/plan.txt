...
**Security**
- [x] Configure security headers in `next.config.ts` — HSTS, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy (excluded from `/api/*` to avoid breaking Telegram webhook/bot endpoints). CSP deferred until Sumit/JGive iframe integration is known.
