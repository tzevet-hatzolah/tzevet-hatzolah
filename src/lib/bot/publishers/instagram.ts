...
async function graphApi(
  ...
  const payload = { access_token: getPageToken(), ...body };
  console.log(`[Instagram] POST ${endpoint}`, JSON.stringify({ ...payload, access_token: "***" }));
  ...
  if (data.error) {
    console.error(`[Instagram] Error:`, JSON.stringify(data.error));
    throw new Error(
