                ...
                <div className="flex flex-wrap gap-3">
                  {socials.map(({ key, label, href }) => (
                    <a
                      ...
                      aria-label={label}
                      className="w-11 h-11 rounded-full bg-navy-400 text-white flex items-center justify-center hover:bg-navy-400/80 hover:scale-105 transition-all duration-300"
                    >
                      <span className="block w-5 h-5">{socialIcons[key]}</span>
                    </a>
                  ))}
                </div>
              )}
